...
public class DiscoveryExample {
    ...
    public static final String PATH = "/discovery";
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            client.start();
            serviceDiscovery = ServiceDiscoveryBuilder.builder(ExampleService.class).client(client).basePath(PATH).build();
            serviceDiscovery.start();

            processCommands(serviceDiscovery, providers, client);
        } finally {
            for (ServiceProvider<ExampleService> cache : providers.values()) {
    ...
    private static void processCommands(ServiceDiscovery<ExampleService> serviceDiscovery, Map<String, ServiceProvider<ExampleService>> providers, CuratorFramework client) throws Exception {
    ...
    private static void listRandomInstance(String[] args, ServiceDiscovery<ExampleService> serviceDiscovery, Map<String, ServiceProvider<ExampleService>> providers, String command) throws Exception {
        ...
        ServiceProvider<ExampleService> provider = providers.get(serviceName);
        if (provider == null) {
            provider = serviceDiscovery.serviceProviderBuilder().serviceName(serviceName).providerStrategy(new RandomStrategy<ExampleService>()).build();
    ...
    private static void listInstances(ServiceDiscovery<ExampleService> serviceDiscovery) throws Exception {
        ...
        try {
            ...
            for (String serviceName : serviceNames) {
                Collection<ServiceInstance<ExampleService>> instances = serviceDiscovery.queryForInstances(serviceName);
                System.out.println(serviceName);
                for (ServiceInstance<ExampleService> instance : instances) {
                    outputInstance(instance);
                }
            }
        } finally {
    ...
    private static void outputInstance(ServiceInstance<ExampleService> instance) {
